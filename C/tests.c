...
void
test_expression (char *expression, double expected)
{
  ok (parse_expression (expression) == expected, "Expression: %s, expected: %f",
      expression, expected);
}

void
_split (char *text, char delimiter, char ***list)
{
...
main (int argc, char *argv[])
{
  ...
  ok (ascii_hash ("nome_tabela") < 8500, "%s",
      "Teste com a palavra nome_tabela");
  ok (ascii_hash ("nova_palavra") < 8500, "%s",
      "Teste com a palavra nova_palavra");
  ...
  if (!file_exists ("connection.test"))
    {
      append_to_file ("connection.test",
		      "mysql:host=localhost;db=test\nroot\nroot");
    }
  ...
  ok (commands[commands_count - 1].line == 5, "%s",
      "Verifica a inclusao do comando");
