...
typedef struct st_database_connection
{
    ...
    char *db;
    MYSQL *conn;
    int (*close)(struct st_database_connection *c);
    int isclosed;
} database_connection;

int _close(struct st_database_connection *c)
{
    if(!c->isclosed)
    {
        mysql_close(c->conn);       
...
void _read_key_value_text(char * text)
{
  char* value;
  char* key;
  char **_array;
 ...
 *
 */
int _mysql_connection_file_reader(int line, char *text)
{
  if(line == 1)
  {
    char ** driver_array;
...
int read_mysql_connection_from_file(char *file_name, struct st_database_connection *c)
{
...
int open_connection(struct st_database_connection *c)
{
    c->conn = mysql_init(NULL);
    if (!mysql_real_connect(c->conn, c->host, c->user, c->pass, c->db, 0, NULL, 0)) {
       fprintf(stderr, "%s\n", mysql_error(c->conn));
       exit(1);
    }
