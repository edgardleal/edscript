...
public final class RandomUserAgent {
	...
	private static class System {
		...
		public System(final String platform, final String osName,
				final String archs, final boolean inverseOrder,
				final boolean useIt) {
	 ...
	 */
	private static String winVersions = Constants.EMPTY_STRING;
	...
	/**
	 * EMPTY Macintosh: version addon-strings, to make the user-agent more
	 * random
	 */
	...
	static {
		...
		RandomUserAgent.winAddons.add(new String[] { Constants.EMPTY_STRING,
				".NET CLR 1.0.3705", ".NET CLR 1.1.4322" });
		RandomUserAgent.winAddons.add(new String[] { Constants.EMPTY_STRING,
				".NET CLR 2.0.40607", ".NET CLR 2.0.50727" });
		RandomUserAgent.winAddons.add(new String[] { Constants.EMPTY_STRING,
				".NET CLR 3.0.04506.648", ".NET CLR 3.0.4506.2152" });
		RandomUserAgent.winAddons.add(new String[] { Constants.EMPTY_STRING,
		...
		RandomUserAgent.macAddons.add(Constants.EMPTY_STRING);
		...
		RandomUserAgent.system.add(new System("Windows", "Windows",
				RandomUserAgent.winVersions, false, true));
		RandomUserAgent.system.add(new System("X11", "Linux",
				"x86|x86_64|i586|i686", false, true));
		RandomUserAgent.system.add(new System("Macintosh", "Mac OS X",
				"Intel|PPC|68K", true, true));
		RandomUserAgent.system.add(new System("X11", "FreeBSD",
				"i386|amd64|sparc64|alpha", false, true));
		RandomUserAgent.system.add(new System("X11", "OpenBSD",
				"i386|amd64|sparc64|alpha", false, true));
		RandomUserAgent.system.add(new System("X11", "NetBSD",
				"i386|amd64|sparc64|alpha", false, true));
		RandomUserAgent.system.add(new System("X11", "SunOS", "i86pc|sun4u",
				false, true));
	}
	...
	private static String dotNetString() {
		...
		String dotNet10 = "; "
				+ RandomUserAgent.winAddons.get(0)[rand
						.nextInt(RandomUserAgent.winAddons.get(0).length)];
		if (dotNet10.equalsIgnoreCase("; ")) {
			dotNet10 = Constants.EMPTY_STRING;
		}

		String dotNet20 = "; "
				+ RandomUserAgent.winAddons.get(1)[rand
						.nextInt(RandomUserAgent.winAddons.get(1).length)];
		if (dotNet20.equalsIgnoreCase("; ")) {
			dotNet20 = Constants.EMPTY_STRING;
		}

		String dotNet30 = Constants.EMPTY_STRING;
		if (dotNet20.length() != 0) {
			dotNet30 = "; "
					+ RandomUserAgent.winAddons.get(2)[rand
							.nextInt(RandomUserAgent.winAddons.get(2).length)];
		}
		if (dotNet30.equalsIgnoreCase("; ")) {
			dotNet30 = Constants.EMPTY_STRING;
		}

		String dotNet35 = Constants.EMPTY_STRING;
		if (dotNet30.length() != 0) {
			dotNet35 = "; "
					+ RandomUserAgent.winAddons.get(3)[rand
							.nextInt(RandomUserAgent.winAddons.get(3).length)];
		}
		if (dotNet35.equalsIgnoreCase("; ")) {
			dotNet35 = Constants.EMPTY_STRING;
		}
	...
	public static String generateFF() {
		...
		String platform = Constants.EMPTY_STRING;
		String osAndArch = Constants.EMPTY_STRING;
		String winAddon = Constants.EMPTY_STRING;
		String linuxAddon = " ";
		String macAddon = Constants.EMPTY_STRING;
		...
		do {
			...
			final String[] archs = RandomUserAgent.system.get(i).archs
					.split("\\|");
		...
		} else if (RandomUserAgent.system.get(i).osName
				.equalsIgnoreCase("Linux")) {
			linuxAddon = RandomUserAgent.linuxAddons.get(rand
					.nextInt(RandomUserAgent.linuxAddons.size()));
			if (linuxAddon != " ") {
				linuxAddon = " " + linuxAddon.trim() + " ";
			}
		} else if (RandomUserAgent.system.get(i).osName
				.equalsIgnoreCase("Mac OS X")) {
			macAddon = RandomUserAgent.macAddons.get(rand
					.nextInt(RandomUserAgent.macAddons.size()));
			if (macAddon != Constants.EMPTY_STRING) {
				macAddon = " " + macAddon.trim();
			}
		}

		/* Get Browser language */
		final String lang = RandomUserAgent.langs.get(rand
				.nextInt(RandomUserAgent.langs.size()));

		/* Get Firefox branch revision, version and release date */
		final String[] tmpFFVersionInfos = RandomUserAgent.ffVersionInfos.get(
				rand.nextInt(RandomUserAgent.ffVersionInfos.size())).split(
				"\\|");
		...
		return "Mozilla/5.0 (" + platform + "; U; " + osAndArch + macAddon
				+ "; " + lang + "; rv:" + ffRev + ") Gecko/"
				+ RandomUserAgent.randomDate(ffReleaseDate) + linuxAddon
				+ "Firefox/" + ffVersion + winAddon;
	}
	...
	public static String generateIE() {
		...
		final String ieVersion = RandomUserAgent.ieVersions.get(rand
				.nextInt(RandomUserAgent.ieVersions.size()));
		final String winVersion = RandomUserAgent.winVersions.split("\\|")[rand
				.nextInt(RandomUserAgent.winVersions.split("\\|").length)];
		String trident = Constants.EMPTY_STRING;
		if (ieVersion.equalsIgnoreCase("8.0")) {
			trident = "; Trident/4.0";
		}

		return "Mozilla/4.0 (compatible; MSIE " + ieVersion + "; Windows "
				+ winVersion + trident + RandomUserAgent.dotNetString() + ")";
	}
	...
	private static String randomDate(final String[] releaseDate) {
		String returnDate = RandomUserAgent.releaseDate[0]
				+ RandomUserAgent.releaseDate[1]
				+ RandomUserAgent.releaseDate[2];
		if (releaseDate == null || releaseDate.length < 3
				|| releaseDate.length > 4) {
			return returnDate;
		}

		final Calendar rCal = new GregorianCalendar(
				Integer.parseInt(releaseDate[0]),
				Integer.parseInt(releaseDate[1]) - 1,
		...
		final Random rand = new Random();
		int temp = (int) ((cTime - rTime) / (60 * 1000))
				+ (int) (rTime / (60 * 1000));
		if (temp < 0) {
			temp = -temp;
		}
		...
		final String month = StringUtil.fillString(
				String.valueOf(rCal.get(Calendar.MONTH) + 1), "0",
				Constants.EMPTY_STRING, 2);
		final String day = StringUtil.fillString(
				String.valueOf(rCal.get(Calendar.DAY_OF_MONTH)), "0",
				Constants.EMPTY_STRING, 2);
		String hour = Constants.EMPTY_STRING;
		if (releaseDate.length == 4) {
			hour = StringUtil.fillString(String.valueOf(rand.nextInt(24)), "0",
					Constants.EMPTY_STRING, 2);
		}
		returnDate = StringUtil.concat(String.valueOf(year), month, day, hour);
		return returnDate;
	}
